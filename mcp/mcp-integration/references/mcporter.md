...
- Config location pitfall: user-wide mcporter config is `~/.mcporter/mcporter.json`; if `mcporter list` says no servers are configured, check that path before assuming MCP itself is broken.
- MCP tool names often include prefixes/suffixes. Always inspect with `mcporter list <server> --schema` before calling; do not guess names like `exa.search` when the actual tool may be `exa.web_search_exa`.
