...
```bash
python3 ~/.hermes/skills/media/ncm-cli-setup/scripts/install_mpv.py
```

**Linux**：询问用户是否需要播放功能：

- 如果是桌面环境、需要本地播放 → 执行 `python3 ~/.hermes/skills/media/ncm-cli-setup/scripts/install_mpv.py`
...
| `mpv not found`              | 重新运行 `python3 ~/.hermes/skills/media/ncm-cli-setup/scripts/install_mpv.py` 或手动安装：https://mpv.io |
...
- 安装位置：Hermes active profile 的 `~/.hermes/skills/media/ncm-cli-setup/`。
- 未运行上游安装脚本；只安装 skill 文档与必要辅助脚本。

### Hermes / WSL 注意

- 当前环境是 WSL/Linux。安装 mpv 需要 `sudo apt-get`，除非用户明确要本地播放，否则不要自动安装 mpv。
- `ncm-cli configure` / `config set privateKey` 涉及密钥；不要在聊天正文回显 privateKey。
