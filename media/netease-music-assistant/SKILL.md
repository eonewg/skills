...
description: >
   ...
   核心能力：分析用户红心歌单偏好画像、制定多关键词搜索策略、智能推荐歌单/专辑/单曲（含两层推荐说明）、播放控制（播放/暂停/下一首/音量/队列）、管理定时推送调度、通过 Hermes 当前消息渠道或 Hermes cron 推送推荐结果、创建并写入歌单。
...
2. **【关键】同步注册到 Hermes cronjob** - 必须执行以下步骤：
   - 先用 `cronjob(action="list")` 检查现有网易云/云音乐相关任务，避免重复创建。
   - 对比 `ncm-schedule.json` 中的规则，确认每个规则都在 Hermes cron 中注册。
   - 对于新规则，用 `cronjob(action="create", schedule=..., prompt=..., skills=["netease-music-assistant"], deliver="origin")` 创建；prompt 必须自包含，说明使用 ncm-cli 生成推荐并发回当前渠道。
   - 再次 `cronjob(action="list")` 验证任务存在。
3. 检测你是否有调度能力
   - Hermes 支持调度时，必须将用户诉求写入 Hermes cronjob，然后输出确认：「已设置：[规则描述]，下次触发：[时间]」。
   - 不要直接改系统 `crontab`，除非用户明确要求系统级 cron。
...
- **【调度关键】更新 `ncm-schedule.json` 后必须同步注册到 Hermes cronjob，否则定时任务不会生效**
   - 检查方法：`cronjob(action="list")`
   - 注册方法：`cronjob(action="create", schedule=..., prompt=..., skills=["netease-music-assistant"], deliver="origin")`
   - 验证方法：再次 `cronjob(action="list")` 确认任务存在
...
- 安装位置：Hermes active profile 的 `~/.hermes/skills/media/netease-music-assistant/`。
