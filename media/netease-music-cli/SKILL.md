...
- `ncm-cli user favorite --output json` 返回的是单元数据（歌单本身），**不含 tracks**。拉歌单曲目必须用 `ncm-cli playlist tracks --playlistId <encryptedId>`。
- `playlist tracks` 的 `--playlistId` 只认 **32 位 hex 加密 ID**，传 originalId（数字）会报 400。
- `playlist tracks` 不支持 `--output json` 参数时直接默认 json；如果返回 400，检查 ID 类型。
- 搜索命令：`ncm-cli search song|playlist|album --keyword xxx --limit N --output json`。
- `user favorite` 返回的 data 结构：`{originalId, id(encrypted), name, jumpUrl, coverImgUrl, trackCount, ...}`。
...
- 来源：NetEase/skills @ `58ff0771d6977feb847d6bfe322280eec4bec004`。
- 安装位置：Hermes active profile 的 `~/.hermes/skills/media/netease-music-cli/`。
- 未运行上游安装脚本；只安装 skill 文档与必要辅助脚本。
