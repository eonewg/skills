...
> the user preference: for simply viewing, browsing, or extracting Xiaohongshu content, default to Kimi WebBridge / the user's real browser login session first. Use this MCP client only as a fallback or when explicitly asked to use xiaohongshu-mcp, publish via API, or do server-side automation. See `references/kimi-webbridge-xiaohongshu.md` for the exact browser workflow and reporting caveats.
