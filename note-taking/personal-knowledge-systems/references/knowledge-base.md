...
当用户指定的知识库名称带有口语化后缀时（例如「ai知识信息知识库」），先用原词 `search_knowledge_base` 搜索；如果无结果，不要立刻判定不存在，去掉通用后缀「知识库 / 库 / 资料库」或改用核心关键词再搜一次。最终以返回的 `kb_name` 精确匹配最接近的用户意图，例如用户说「ai知识信息知识库」而实际 `kb_name` 可能是「AI知识信息」。

**ID 解析兜底：** 某些环境下 `search_knowledge_base` 能返回目标知识库名称，但返回结构里没有可直接用于写入的 `id` / `knowledge_base_id`。如果当前任务是“添加/导入内容”且已通过搜索确认目标名称存在，可以调用 `get_addable_knowledge_base_list` 作为写入 ID 解析兜底，从可添加列表中按精确 `kb_name` / `name` 匹配目标。这个兜底只用于已确认名称的添加流程，不替代用户未指定目标知识库时的选择流程。

添加微信文章/网页到已定位知识库时，直接调用 `openapi/wiki/v1/import_urls`，成功后用 `search_knowledge` 在目标知识库内按文章标题或关键词验证条目已出现；面向用户只报告知识库名和文章标题，不暴露 `knowledge_base_id` / `media_id`。
