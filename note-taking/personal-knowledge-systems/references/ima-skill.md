...
如果当前环境里找不到 `ima_api.cjs`，可以直接调用官方 HTTP 接口作为兜底：`POST https://ima.qq.com/<apiPath>`，JSON body 使用 UTF-8，headers 必须包含 `ima-openapi-clientid: <client_id>`、`ima-openapi-apikey: <api_key>`、`Content-Type: application/json`。凭证优先从环境变量读取，缺失时可读 `~/.config/ima/client_id` 和 `~/.config/ima/api_key`。该兜底只允许发往 `ima.qq.com`，不要把凭证传给其他域名。

### the user 微信文章导入验证补充

当用 `openapi/wiki/v1/import_urls` 把微信文章导入 the user 的知识库时，`code=0` 且返回每个 URL 的 `ret_code=0` / `media_id` 就代表导入请求已成功提交。不要把紧接着调用 `search_knowledge` 返回 0 条误判为导入失败；IMA 知识库索引可能有延迟。即时验证优先记录/检查 `media_id`，必要时等待后重试搜索，最终回复中可说明“导入成功，搜索索引可能延迟”。

the user 的 IMA 知识库路由：AI / Agent / 大模型 / AI Infra 文章导入「AI知识信息」；个人成长、心理自护、能量管理、关系边界、个人操作系统文章导入「个人成长与自我照顾」。如果需要新建知识库，当前 IMA OpenAPI 支持未在公开文档列出的 `openapi/wiki/v1/create_knowledge_base`：body 至少包含 `kb_name`、`name`、`description`、`type`，其中 `type` 可用 `KBT_SHARED_KB`；成功返回 `data.id` 和 `data.name`。导入 URL 到根目录时优先省略 `folder_id`，不要把 `knowledge_base_id` 当根目录 folder_id 传入，实测会报“文件夹不存在”。
