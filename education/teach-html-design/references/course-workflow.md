...
Use this reference when continuing a course in `~/teach/<topic>/`.
...
```bash
python3 ~/teach/_templates/make-standalone.py ~/teach/<topic>/lessons/000X-<topic>.html
python3 ~/teach/_templates/validate-template.py ~/teach/<topic>/lessons/000X-<topic>.html
```
...
Then browser-open the exact standalone file and verify visible colors plus at least one JS interaction before sending `MEDIA:/absolute/path.html`.
