...
- Prefer typography, whitespace, and weak horizontal separators over rounded cards, pills, panels, decorative gradients, or repeated vertical rules.
...
Avoid full accent-colored blocks and avoid using a left vertical rule by default.
Preferred callout treatment:
- transparent background
- no full border, no rounded rectangle
- accent-colored strong label at the start
- use `.callout.rule` with `border-left: 3px solid var(--accent)` only for one or two genuinely high-signal blocks per page; never make every example/quiz/callout a vertical-line block

### Formula and example blocks

Avoid full bordered rounded cards and title color bands.
Preferred:
- borderless editorial flow or a weak horizontal separator
- small accent heading line
- examples/quizzes should not have default `border-left`; preserve hierarchy with title color, spacing, and content rhythm
...
- callout/example/quiz blocks do not use default left borders; any left-rule callout is rare and explicit (`.callout.rule`)
