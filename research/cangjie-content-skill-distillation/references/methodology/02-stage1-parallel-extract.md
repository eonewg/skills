...
- **速度**: Hermes 的 subagent/delegation 工具可并行运行；当前环境并发上限可能是 3，所以必要时分两批执行。
...
优先通过 Hermes delegation 分批并行执行；若并发上限为 3，则先跑 framework/principle/case，再跑 counter-example/glossary。没有 delegation 时也可以串行，但必须保持五路输出彼此独立。
