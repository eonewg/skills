...
- **Coordinate-first / preferred for real searchable PDFs:** use `scripts/ocrmypdf_searchable_pdf.sh`, which wraps OCRmyPDF + Tesseract. This produces a text layer with word/line coordinates that tracks scanned glyphs closely, so search, selection, and copy/paste behave like a normal OCRed book.
- **Official PaddleOCR-VL extraction / preferred for structured Markdown:** use `scripts/paddleocr_official_job.py`, which calls Baidu AI Studio's official `PaddleOCR-VL-1.6` async job API, downloads JSONL, page Markdown, combined Markdown, and extracted images. Token comes from `PADDLEOCR_TOKEN`, `--token`, or local private env file `a local private env file such as ~/.config/paddleocr/env`.
- **Legacy SiliconFlow VLM fallback:** use `scripts/siliconflow_pdf_ocr.py` only if the official API is unavailable. SiliconFlow chat-completions returns text/Markdown rather than word-level boxes, so it is not the preferred PDF text-layer engine.
...
When a user sends a PDF and asks for OCR, process it without asking extra questions unless the file is missing or the OCR pipeline is blocked. Default delivery is only the processed searchable PDF:
...
3. `a local private env file such as ~/.config/paddleocr/env`
...
```bash
python3 \
  scripts/paddleocr_official_job.py input.pdf \
  -o input_paddleocr \
...
```bash
# Set SILICONFLOW_API_KEY in your shell or secret manager before running.
scripts/siliconflow_pdf_ocr.py input.pdf
```
...
1. **Hardcoding tokens.** Keep PaddleOCR token in `a local private env file such as ~/.config/paddleocr/env` or environment variables, not in skills/scripts.
