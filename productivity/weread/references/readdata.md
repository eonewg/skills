...
2. **用户问“最近阅读情况 / 最近读得怎么样”**：同时查询 `mode=weekly` 和 `mode=monthly`。先用月度给整体趋势、读过/读完/笔记/读得最多，再用周度判断最近几天是否断档；如果周度 `readDays=0` 但有几十秒阅读，说明未达到微信读书“单日满 1 分钟才算有效阅读日”的口径，不要误说完全没有打开。
3. **用户问本周/今年/总共**：对应传 `mode=weekly`/`annually`/`overall`。
