---
name: weread
description: "Use when accessing 微信读书 / WeRead through the Agent API Gateway: search books, inspect shelf, reading stats, notes/highlights, reviews, recommendations, and profile data."
version: 1.0.3
source: https://cdn.weread.qq.com/skills/weread-skills.zip
source_sha256: 39f8f4cb7cbf750eec6390893d5427a0fcd022465169138fbd1df431f70bfac9
author: WeRead / Hermes local installation
license: NOASSERTION
metadata:
  hermes:
    tags: [weread, wechat-reading, books, notes, highlights, reading-stats]
    related_skills: [personal-knowledge-systems, obsidian]
---
# WeRead — 微信读书助手
...
| 阅读分析报告 | 组合阅读统计、书架、笔记本与进度数据，生成深度阅读画像；需要邮件投递时制作 HTML + 纯文本 fallback 并用邮件技能发送 | "根据我的微信读书统计为我做阅读分析报告并发邮件" | `references/reading-analysis-email-report.md` |
...
- Original upstream skill name: `微信读书`。Hermes 本地安装名规范化为 `weread`，便于技能加载。
- Source: `https://cdn.weread.qq.com/skills/weread-skills.zip`
- Source SHA256: `39f8f4cb7cbf750eec6390893d5427a0fcd022465169138fbd1df431f70bfac9`
- Supporting documents from the zip are installed under `references/`.
- This skill calls `https://i.weread.qq.com/api/agent/gateway` and requires `WEREAD_API_KEY` in the environment. Do not hardcode or reveal the API key.
