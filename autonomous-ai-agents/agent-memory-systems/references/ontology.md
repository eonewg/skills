...
# Import
import sys
sys.path.insert(0, 'skills/ontology')
from ontology import *

# Create entities (one-liner)
person('Ethan', goal='<exam-target>考研', role='考研党')
note('镜的刷经济技巧', content='多吃中线', source='宗师直播')
project('2026考研', status='active', target='<exam-target>')

# Create relations (one-liner)
relate('ethan', 'working_on', '2026考研')
relate('ethan', 'learned', '镜的刷经济技巧')
relate('镜的刷经济技巧', 'related_to', '打野意识', strength='high')

# Query (one-liner)
ethan = get('ethan')                           # Get by ID
results = find('镜')                            # Search
results = find(entity_type='Note')             # Filter by type
rels = relations('ethan')                      # Get all relations
related_entities = related('ethan')            # Follow outgoing relations
```
